...
struct ip_hdr {
  uint8_t vhl; // バージョン（4bit）と IPヘッダ長（4bit）をまとめて 8bit として扱う
  uint8_t tos;
  uint16_t total;
  uint16_t id;
  uint16_t offset; // フラグ（3bit）と フラグメントオフセット（13bit）をまとめて 16bit として扱う
  uint8_t ttl;
  uint8_t protocol;
  uint16_t sum;
  ip_addr_t src;
...
/* NOTE: must not be call adter net_run() */
...
static void ip_input(const uint8_t *data, size_t len, struct net_device *dev) {
  ...
  if (cksum16((uint16_t *)data, hlen, 0)) {
  ...
  if (!iface) {
    ...
    errorf("no iface, dev=%s", dev->name);
    return;
  }
  if (hdr->dst != iface->unicast) {
    if (hdr->dst != iface->broadcast && hdr->dst != IP_ADDR_BROADCAST) {
      /* for other host */
      errorf("not for us, dev=%s, dst=%s", dev->name, ip_addr_ntop(hdr->dst, addr, sizeof(addr)));
      return;
    }
  }
